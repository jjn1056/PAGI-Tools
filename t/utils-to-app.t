...
is(to_app(Local::App->new), $compiled, 'an app object compiles');
is($Local::App::CALLS, 1, 'an app object compiles exactly once');
...
like(dies { to_app([]) }, qr/native coderef or app object.*instantiated object with to_app/i,
    'unblessed references are not apps');
like(dies { to_app(undef) }, qr/native coderef or app object.*instantiated object with to_app/i,
    'undefined is not an app');
ok(!Local::UtilsDefaultImports->can('as_app_object'),
    'as_app_object is not exported by default');
