...
subtest 'auto negotiation selects the documented representation families' => sub {
    ...
    my $exact_problem_rejection = send_response(PAGI::Pages->not_found(
...
subtest 'fixed representation ignores Accept and does not add Vary' => sub {
    my $events = send_response(PAGI::Pages->new(as => 'json')->not_found(
        http_scope(accept => 'text/html'), as => 'text'));
...
subtest 'welcome is not a problem document and preserves exact stock copy' => sub {
    my $problem_only = send_response(PAGI::Pages->welcome(
        http_scope(accept => 'application/problem+json')));
    is(header($problem_only, 'Content-Type'), 'text/html; charset=utf-8',
        'problem+json alone does not select welcome JSON');

    my $json_events = send_response(PAGI::Pages->welcome(
        http_scope(accept => 'application/json')));
...
subtest 'stock error text, HTML escaping, and UTF-8 length are stable' => sub {
    my $stock = send_response(PAGI::Pages->not_found(http_scope(), as => 'text'));
    ...
    my $unicode = send_response(PAGI::Pages->not_found(
        http_scope(), as => 'text', detail => "Caf\x{e9} \x{2615}"));
...
subtest 'problem JSON has standard members, optional instance, and extensions' => sub {
    my $events = send_response(PAGI::Pages->not_found(
        http_scope(), as => 'json', instance => '/requests/abc',
...
subtest 'stock favicon is inline exact-status SVG with documented seams' => sub {
    my $registered_html = decode('UTF-8', body(send_response(
        PAGI::Pages->not_found(http_scope(), as => 'html'))), FB_CROAK);
    ...
    my $omitted = decode('UTF-8', body(send_response(
        Local::NoFaviconPages->not_found(http_scope(), as => 'html'))), FB_CROAK);
...
subtest 'returned Responses remain caller-mutable before respond' => sub {
    my $response = PAGI::Pages->not_found(http_scope(), as => 'text');
    $response->header('X-Request-ID', 'request-17');
    my $events = send_response($response);
    is(header($events, 'X-Request-ID'), 'request-17',
        'caller mutation is preserved when the Response is later sent');
};
