...
The final `compose(router => $routing, lifespan => { ... })` expression is an
inspectable root application. Compose owns startup/shutdown, error handling,
and response completion; its retained Router owns negotiated 404/405 and
automatic HEAD behavior.
