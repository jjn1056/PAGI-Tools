...
=item 3. C<await $send-E<gt>({...})> - Sends events asynchronously, waiting for each to complete

=item 4. C<\&app> - Returns a reference to the application subroutine
...
=item * L<Mojo::Pg> (with Mojo::IOLoop) for PostgreSQL
...
Offload blocking operations to a worker process:
...
=head3 When to Use Worker Mode
  ...
  -----------------------------------|----------------|-------------
  Pure async app (no blocking)       | ✓              | ✓
  App uses some blocking operations  | ✗              | ✓
  High CPU usage per request         | ✗              | ✓
  WebSocket/SSE with shared state    | ✓              | Needs PubSub
  Development/testing                | ✓              | Optional
  Production (multi-core server)     | Optional       | ✓
