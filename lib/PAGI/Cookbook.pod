...
=head3 Basic Router Class
  ...
  async sub about {
      my ($self, $req, $res) = @_;
      return $res->html('<h1>About Us</h1>');
  }
...
Define middleware methods and reference them by name:
  ...
  async sub admin_page {
      my ($self, $req, $res) = @_;
      return $res->html('<h1>Admin Panel</h1>');
  }
...
Create modular applications by mounting sub-routers:
  ...
  async sub create_user {
      my ($self, $req, $res) = @_;
      my $data = await $req->json;
      return $res->status(201)->json({ id => 3, name => $data->{name} });
  }
...
Here's a complete working example demonstrating all features:
  ...
  async sub home {
      my ($self, $req, $res) = @_;
      return $res->html('<h1>Welcome to MyApp</h1>');
  }

  async sub list_users {
      my ($self, $req, $res) = @_;
      return $res->json($self->state->{users});
  }
...
=head2 Bearer Token (JWT)
  ...
  $router->get('/api/me' => async sub {
      ...
      await $res->json({ user => $user })->respond($send);
  });
...
"lost future" warnings:
  ...
  $router->post('/signup' => async sub {
      ...
      # Schedule background work (non-blocking; runs while response is sent)
...
=head2 File Uploads (Multipart)
  ...
  async sub app {
      ...
      if ($req->method eq 'POST') {
          ...
          await $res->json({ uploaded => scalar(@$uploads) })->respond($send);
      }
      else {
          await $res->html(<<'HTML')->respond($send);
