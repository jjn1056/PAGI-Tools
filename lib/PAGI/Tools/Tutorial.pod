...
automatic OPTIONS. Inline mounts are structural; positional mounts are opaque
applications that own a prefix. Pure route/mount/router middleware is described
by a bare factory in its C<middleware> list, such as
...
three-channel endpoints remain explicit with C<raw>. See L<PAGI::Routing> for the complete lifecycle and
