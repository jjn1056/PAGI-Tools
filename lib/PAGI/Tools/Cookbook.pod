...
L<PAGI::App::Router>, and the class-based L<PAGI::Endpoint::Router>. For a
...
Passing a Router positionally chooses that same opaque contract:
  ...
  mount('/known', router => $people, namespace => 'person') # inspectable child

C<< mount('/opaque' => $people) >> is not shorthand for the named
C<< router => $people >> form.
Opaque mounts reject C<namespace>; inline mounts may omit it; Router mounts
require one local logical segment.
...
Names and namespaces are local logical segments; slash alone forms hierarchy.
Inline namespaces are optional, while explicit Router mounts require one:

  my $routing = router(routes => [
      mount('/api/v1', routes => $shared, namespace => 'v1'),
      mount('/api/v2', routes => $shared, namespace => 'v2'),
  ]);
...
C<lifespan.startup> and C<lifespan.shutdown> protocol events -- make Compose
...
Immediate values and Future-backed callback completion are both accepted. The
state hash is supplied and propagated to request scopes by the server; Compose
does not create a private substitute.
