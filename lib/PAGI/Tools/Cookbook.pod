...
All constraints receive one decoded scalar and validate without coercion:
  ...
  route('/typed/{id}' => \&show,
      constraints => { id => $type_tiny_compatible_object },
  );

The final object needs C<check($value)> and may provide
C<get_message($value)>; Type::Tiny is not a hard dependency. Regex and inline
...
this exact C<match> before route middleware:

  {
      ...
      name              => '/tenant/user/show', # absolute or undef
