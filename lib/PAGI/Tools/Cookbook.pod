...
Inline mounts append declared path/namespace/description records. A selected
leaf sets the effective kind, pattern, name, and description in C<match> before
route middleware. A generated 404/405 leaves it undefined. A separately
compiled compatible router appends a frame, so child middleware can inspect
ancestry while the parent keeps its terminal mount record.
