...
=head2 Inline and Application Mounts

The C<routes> form is structural. Its names, fallbacks, and reverse paths are
known to the parent:
...
symlinks are handled. Prefer a dedicated static/file component whose
containment policy you have verified.
...
Mount namespaces are optional and affect lookup names, not paths:
...
An unnamed mount leaves child names unprefixed. Duplicate effective names
croak with both paths and namespace guidance. Router C<path_for> has no request
prefix; Context C<path_for> includes the compiled-router entry C<root_path>.
C<url_for> adds a scheme and authority validated by L<PAGI::Authority>.
...
Recurse into inline C<< $mount->routes >>; application mounts are opaque.
...
An inline mount appends this exact record before its middleware runs:
...
this exact C<match> before route middleware:

  {
      ...
      route => '/tenants/{tenant_id}/users/{user_id}', # effective route
      name  => 'tenant.user.show',                     # effective or undef
...
C<name =E<gt> undef>, and its declared C<desc> or undef. It does not append an
inline C<mounts> record for itself. A generated 404/405 leaves C<match>
undefined. See L<PAGI::Routing/MATCHED-ROUTE SCOPE CONVENTION> for the complete
public schema.

A separately compiled compatible router appends a frame, so child middleware
can inspect ancestry while the parent keeps its terminal mount record.
