...
- header lookup — ALREADY -> t/request-header.t: header lookup returns the last field value.
...
- session accessor; session dies without middleware; has_session — ALREADY -> t/session.t: session accessor contract.
...
- ctx->response is a detached accumulator; ctx->respond guards double-send — DELETE -> accumulator/double-send sugar; Response is a value and Compose/server owns cross-emission enforcement.
...
- App Router HTTP handlers receive one Request while Context remains available — ALREADY -> t/endpoint/01-http-constructor.t: endpoint receives a Request.
- App Router WebSocket and SSE handlers receive direct protocol objects — ALREADY -> t/endpoint/02-websocket-constructor.t: endpoint receives a WebSocket; t/endpoint/03-sse-constructor.t: endpoint receives an SSE.
...
- each_message delegates; each_text delegates; each_bytes delegates; each_json delegates — ALREADY -> t/websocket/07-iteration.t: each_message iterates until disconnect; each_text iterates text frames; each_json iterates and decodes.
...
- header_all delegates; http_version delegates — ALREADY -> t/websocket/01-constructor.t: WebSocket scope metadata accessors.
...
- send delegates; send_json delegates; send_event delegates; send_comment delegates — ALREADY -> t/sse/04-send.t: send sends data-only event; send_json encodes as JSON; send_event with all fields; t/sse/04-comment.t: send_comment sends comment event.
- try_send delegates; try_send_json delegates; try_send_comment delegates; try_send_event delegates — ALREADY -> t/sse/05-safe-send.t: try_send returns true on success; try_send_json works; try_send_event works; t/sse/05-comment.t: try_send_comment works.
...
- keepalive delegates — ALREADY -> t/sse/08-keepalive.t: keepalive sends sse.keepalive event.
- query_param delegates; query_params delegates; raw_query_param delegates; raw_query_params delegates — ALREADY -> t/sse/01-constructor.t: SSE query accessors.
- header_all delegates; http_version delegates — ALREADY -> t/sse/01-constructor.t: SSE scope metadata accessors.
...
- all built-in Context subclasses inherit routing reverse methods — TRANSFER -> t/routing/13-url-helper.t: raw and protocol-object sources resolve their exact scopes.
...
- Context reverses supplied child-boundary frames across protocols — TRANSFER -> t/routing/13-url-helper.t: raw and protocol-object sources resolve their exact scopes.
...
- url_for uses validated Host and only absent Host permits server fallback; documented HTTP proxy and Host middleware order feeds routing URL generation; url_for maps the request scheme according to the named route kind — ALREADY -> t/routing/13-url-helper.t: authority validation and target kinds preserve HTTP HTTPS WS WSS mappings.
